...
// this code has the same functionality but is maybe harder to follow logically, or understand what the output is in certain scenarios

/* 
    /// addition with checks when MAX or MIN are exceeded
    pub fn checked_add(self, other: Self) -> Result<Self, OverflowError> {        

        match self.negative == other.negative {
            true => {
                let result = Integer {
                    value:  match self.value.checked_add(other.value) {
                        Ok(v) => v,
                        Err(_) => return Err(OverflowError{operation: Add, operand1: self.to_string(), operand2: other.to_string()}),
                    },
            ...
            false => 
                if self.value > other.value {
                    let result = Integer {
                        value:  match self.value.checked_sub(other.value) {
                            Ok(v) => v,
                            Err(_) => return Err(OverflowError{operation: Add, operand1: self.to_string(), operand2: other.to_string()}),
                        },
                        negative: self.negative,
                    ...
                    let result = Integer {
                        value: match other.value.checked_sub(self.value) {
                            Ok(v) => v,
                            Err(_) => return Err(OverflowError{operation: Add, operand1: self.to_string(), operand2: other.to_string()}),
                        },
                        negative: !self.negative,
    ...
    /// subtraction with checks when MAX or MIN are exceeded
    pub fn checked_sub(self, other: Self) -> Result<Self, OverflowError> {
    ...
    pub fn checked_mul(self, other: Self) -> Result<Self, OverflowError> {
        ...
        let result = Integer {
            value:  match self.value.checked_mul(other.value) {
                Ok(v) => v,
                Err(_) => return Err(OverflowError{operation: Mul, operand1: self.to_string(), operand2: other.to_string()}),
            },
